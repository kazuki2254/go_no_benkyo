...
func encjson(v1, v2, op, res string) []byte {
...
func handle(c echo.Context) error {
	...
	//		return c.Blob(http.StatusOK,"application/json",
	//			[]byte(fmt.Sprintf("{\"%v%v%v\":\"%v\"}",val1,ope,val2,res)))
	return c.Blob(http.StatusOK, "application/json",
		encjson(c.Param("v1"), c.Param("v2"), c.Param("op"), kekka))
}
...
func poshandle(c echo.Context) error {
	...
	if se {
		...
		return c.Blob(http.StatusOK, "application/json", encjson(fr.Val1, fr.Val2, fr.Op, kekka))
